...
import { getdailystats, getorders, getproducts, getcategories } from '../../data/datafetch.js';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [stats, orders, products, categories] = await Promise.all([
          getdailystats(),
          getorders(),
          getproducts(),
          getcategories(),
  ...
  if (sortedStats.length > 0) {
    ...
    statsSlice.forEach((stat) => {
      ordersByDate[stat.date] = { date: stat.date, orders: 0, revenue: 0 };
      visitorsData.push({ date: stat.date, visitors: stat.visitors || 0 });
    ...
    ordersSlice.forEach((order) => {
      if (!ordersByDate[order.date]) {
        ordersByDate[order.date] = { date: order.date, orders: 0, revenue: 0 };
      }
    ...
    conversionRate = totalVisitors ? (totalOrders / totalVisitors) * 100 : 0;
    ...
    ordersLineData = [
      ...orderedDates.map((item) => ({ date: item.date, metric: 'Orders', value: item.orders })),
      ...orderedDates.map((item) => ({ date: item.date, metric: 'Revenue', value: item.revenue })),
    ];
    ...
    ordersSlice.forEach((order) => {
      (order.items || []).forEach((item) => {
        ...
        soldByCategoryMap[name] = (soldByCategoryMap[name] || 0) + (item.qty || 1);
      });
    });
  ...
  const lineConfig = { data: ordersLineData, xField: 'date', yField: 'value', seriesField: 'metric', smooth: true, height: 260 };
  const visitorsConfig = { data: visitorsData, xField: 'date', yField: 'visitors', columnWidthRatio: 0.6, height: 260 };
  const pieConfig = { data: conversionData, angleField: 'value', colorField: 'type', radius: 0.8, innerRadius: 0.6 };
  const categoryPieConfig = { data: soldByCategory, angleField: 'count', colorField: 'name', radius: 0.8 };
  const countryScatterConfig = { data: countryPoints, xField: 'country', yField: 'orders', size: 6, colorField: 'country' };
    ...
    <div className={style.container}>
      ...
      <div className={style.grid}>
        <div className={style.card}>
          ...
          <div>
            ...
            <div className={style.summaryValue}>${totalRevenue.toLocaleString()}</div>
          </div>
          <div className={style.chart}><Line {...lineConfig} /></div>
        </div>
        ...
        <div className={style.card}>
          ...
          <div>
            ...
            <div className={style.summaryValue}>{totalVisitors.toLocaleString()}</div>
          </div>
          <div className={style.chart}><Column {...visitorsConfig} /></div>
        </div>
        ...
        <div className={style.card}>
          ...
          <div>
            ...
            <div className={style.summaryValue}>{conversionRate.toFixed(1)}%</div>
          </div>
          <div className={style.chart}><Pie {...pieConfig} /></div>
        </div>
      </div>
      ...
      <div className={style.grid}>
        <div className={style.card}>
          ...
          <div className={style.chart}><Pie {...categoryPieConfig} /></div>
        </div>
        <div className={style.card}>
          <h3 className={style.title}>Orders by country</h3>
          <div className={style.chart}><Scatter {...countryScatterConfig} /></div>
        </div>
      </div>
    </div>
  );
